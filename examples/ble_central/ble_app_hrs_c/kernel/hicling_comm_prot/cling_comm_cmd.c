...
* @return  none
*/
void cling_comm_cmd_device_over_the_air_update()
{
    uint8_t buf[2];
	// PROTOCOL_MESSAGE_START_OTA
	buf[0] = PROTOCOL_MESSAGE_START_OTA;
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_START_OTA, buf, 1);
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_update_weather_forcast(char *data, uint16_t len)
{
    #define __WEATHER_FORCAST_LEN_MAX__ 200
    uint8_t buf[__WEATHER_FORCAST_LEN_MAX__];
    len = (len > __WEATHER_FORCAST_LEN_MAX__)?__WEATHER_FORCAST_LEN_MAX__:len;
 #if  0
    int len;
...
#endif
    //len = numOfDays*sizeof(cling_weather_contex_t);
	// update weather message
	buf[0] = PROTOCOL_MESSAGE_WEATHER;
    memcpy(buf+1, data, len);
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_WEATHER, buf, len);
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_update_user_reminder(uint8_t *data, int numOfReminders)
{
    uint8_t buf[200];
    int len; 
    len = numOfReminders << 1; // each reminder 2 bytes, HH:MM
    ...
    for (int i = 0; i < numOfReminders; i++) {
        DEBUG("[USER reminder] %d, %d:%d", i, data[0+i*2], data[1+i*2]);
    }
#endif
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_update_simulation(uint8_t *data)
{
    uint8_t buf[200];
    ...
    buf[1] = data[0];
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_USER_REMINDER, buf, 2);
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_set_ancs(uint8_t *data)
{
    uint8_t buf[5];
	// Load device info
    ...
    buf[2] = data[1];
    buf[3] = data[2];
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_SET_ANCS, buf, 5);
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_set_smart_notification(uint8_t * data )
{
    uint8_t buf[5];
	//ENTER_FUNC();
    ...
    buf[2] = data[1]; // id
    buf[3] = data[2]; // counts
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_SET_SMART_NOTIFICATION, buf, 5);
...
* @return  none
*/
void cling_comm_cmd_device_setup(uint8_t *data, int len)
{
     uint8_t buf[32];
    ...
    memcpy(buf+1, data, len);
    comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_DEVICE_SETTING, buf, len+1);
}
/*********************************************************************
...
void cling_comm_cmd_device_reboot()
{
	...
	comm_protocol_hal_send_packet(PROTOCOL_MESSAGE_REBOOT, buf, 1);
}
/*********************************************************************
...
* @return  none
*/
void cling_comm_cmd_write_one_file(uint8_t *f_name, uint8_t *data, uint32_t f_len)
{
	uint32_t msg_len;
...
* @return  none
*/
void cling_comm_cmd_read_one_file(uint8_t *f_name)
{
	uint32_t msg_len;
	uint8_t string_len = (uint8_t)(strlen((char *)f_name)+1);
	uint8_t *msg;
